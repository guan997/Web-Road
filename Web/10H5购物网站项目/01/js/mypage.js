$(function () {
    ...
    $('#fullpage').fullpage({// fullpage()方法接受的是json对象
        ...
        afterLoad: function (anchorLink, index) {
            if (index == 2) {
                // 往第2屏幕滚动时next先消失，等所有动画显示，next淡出
                $(".next").fadeOut();
                // 缓动动画要加到时间的后面，函数的前面
        ...
        onLeave: function (index, nextIndex, direction) {
            ...
            if (index == 3 && nextIndex == 4) {
                ...
                $(".t1f").show().animate({ "bottom": -((k - 250) + 50), "left": 260, }, 2000, function () {
                    ...
                    $(".car").animate({ "left": 1500 }, 3000, "easeInElastic", function () {
                        $(".note").show();
                        $(".note-img, .words-4-a").animate({ "opacity": 1 }, 1000, function() {
                            $(".next").fadeIn();
                        });
                    });
                })
            ...
            // 当点击再来一次的时候，分两步进行
            $(".again").click(function(ecent) {
                // 1.返回第1屏幕
                $.fn.fullpage.moveTo(1);
                // 2. 所有的动画都复原 (核心原理 让动画的元素清除行内元素)
                // 所有动画的盒子添加  move清除
                $("img,.move").attr({"style": ""});
             })
        }
